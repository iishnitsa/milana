...
Верни только готовую сводку, без любых вступительных слов, пояснений или заключительных фраз. Начни сразу с первого предложения сводки.
'''
...
gigo_critic = 'критик'
gigo_questions = 'Пользователь пришлёт тебе задачу. В ответ напиши вопрос или вопросы, какой информации нехватает для выполнения задачи. По одному вопросу на строку. Задавай сразу все вопросы в одном сообщении, если вопросов несколько. Каждый вопрос должен быть самодостаточным, так как система не учитывает контекст при поиске. Например, "Какие существуют авторитетные источники, исследующие феномен X?" вместо "Какие существуют авторитетные источники?". Пришли только вопросы, никакого другого другого текста быть не должно'
...
Собеседник не получит сообщение содержащее правильно написанную и распознанную системой команду.
...
rag_context_marker = """
Извлечённые фрагменты из долгосрочной памяти (RAG).
Эти фрагменты были найдены по смыслу и могут содержать важные детали из более ранних частей диалога, которые не вошли в последние сообщения.
Используйте их как дополнительный источник фактов и контекста, но помните, что они могут быть неполными или не отсортированы по времени.
Используй эти фрагменты для уточнения ответа, но не противоречь явной истории последних сообщений.
"""

global_summary_marker = "\nГлобальная сводка диалога (общая картина, ключевые решения и факты из всего разговода):\n"
