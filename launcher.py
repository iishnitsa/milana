...
def hide_console():
    try:
        if sys.platform == "win32":# Получаем дескриптор консоли
            console_window = ctypes.windll.kernel32.GetConsoleWindow()
            if console_window: ctypes.windll.user32.ShowWindow(console_window, 0)  # SW_HIDE
    except: pass

# Вызываем сразу при импорте
hide_console()

def show_error(title, message): # Показывает сообщение об ошибке в диалоговом окне
    try:
        if sys.platform == "win32": ctypes.windll.user32.MessageBoxW(0, message, title, 0x10)  # MB_ICONERROR
        else:
            root = tk.Tk()
            root.withdraw()
            messagebox.showerror(title, message)
            root.destroy()
    except: print(f"ERROR: {title} - {message}", file=sys.stderr)

def show_info(title, message): # Показывает информационное сообщение
    try:
        if sys.platform == "win32": ctypes.windll.user32.MessageBoxW(0, message, title, 0x40)  # MB_ICONINFORMATION
        else:
            root = tk.Tk()
            root.withdraw()
            messagebox.showinfo(title, message)
            root.destroy()
    except: print(f"INFO: {title} - {message}", file=sys.stderr)
...
def run_as_interpreter(): # Режим исполнения ui.py внутри среды лаунчера
    ...
    try: # Подготавливаем загрузку ui.py как главного модуля
...
def get_pythonw_path():
    ...
    if sys.platform != "win32": return sys.executable
...
def launch_external_ui(): # Запуск ui.py без консольного окна
...
def main():
    ...
    hide_console()
    multiprocessing.freeze_support() # Фикс для работы multiprocessing в упакованном EXE
    # Проверка: если это дочерний процесс multiprocessing
    if len(sys.argv) > 1:
        arg = sys.argv[1].lower()
        if arg == '--multiprocessing-fork' or arg.startswith('--multiprocessing'): return
    # Если передан .py файл — запускаем его
    if len(sys.argv) > 1 and sys.argv[1].endswith(".py"):run_as_interpreter()
    else: # Проверяем окружение перед запуском
        if not check_environment(): # Если проверка не пройдена, показываем сообщение и ждем
            if sys.platform == "win32": ctypes.windll.user32.MessageBoxW(0, "Программа будет закрыта.", "Ошибка проверки окружения", 0x10)
            sys.exit(1)
