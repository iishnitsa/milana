...
def main(quest):
    if not hasattr(main, 'attr_names'):
        ...
        main.answer_l_2 = """You must determine whether the provided answer (fragments) fully satisfies the given query.
If the answer completely and exhaustively addresses the query, output exactly one line containing only the digit "1" and nothing else.
If the answer is incomplete, insufficient, or does not address the query at all, you must formulate one or more reformulated queries (each on a separate line) that would help find the missing information. These queries should be specific, clear, and aimed at retrieving precisely the details that are lacking.
Important: do not output any explanations, comments, introductory or concluding phrases. Only either "1" or the new queries (each on a new line).

Examples of correct responses (when the answer is incomplete):
What is the melting point of gold?
What are the mechanical properties of titanium?

Examples of incorrect responses:
"Missing information about the melting point of gold."
"I couldn't find data, try refining: melting point of gold? properties of titanium?"
"1. What is the melting point of gold? 2. What are the mechanical properties of titanium?" (here numbering and punctuation are used, which is not allowed)

If the answer fully satisfies the query, the response must be strictly: 1
"""
        main.best_result_system = """Which of these fragments is most relevant to the query?
Return only the most relevant fragment itself, without any additional text, explanations, or introductory phrases.
Do NOT write things like "The most relevant fragment is:", "Here is the answer:", or any other commentary.
Just output the fragment exactly as it appears, nothing else.
""" # TODO: может цифры просто присылать
        ...
        while attempts_left > 0:
            ...
            for i, emb in zip(requests_to_libraries, embeddings):
                ...
                if items:
                    ...
                    useful_items = []
                    for it in items:
                        prompt_answer = (main.label_query + '\n' + i + '\n' + main.label_answer + '\n' + it['text'])
                        answer_val = parse_prompt_response(main.answer_l, prompt_answer, 1)
                        if answer_val == 1: useful_items.append(it)
                    # Выделение важного
                    if useful_items:
                        combined_text = '\n'.join(it['text'] for it in useful_items)
                        user_select = (main.label_answer + '\n' + combined_text)
                        try: selected_text = ask_model(user_select, system_prompt=main.select_matter_l + main.label_query + '\n' + i)
                        except: selected_text = ask_model(text_cutter(user_select), system_prompt=main.select_matter_l + main.label_query + '\n' + i)
                        # Сохраняем источник от первого полезного элемента
                        if useful_items: useful_items = [{'text': selected_text, 'source': useful_items[0]['source']}]
