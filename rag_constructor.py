...
def create_hierarchical_summary(chat_id: str, messages: list[dict], summary_type: str, token_threshold: int = 2000):
    ...
    if not messages: let_log("Нет сообщений для суммаризации"); return
    chunks = []
    current_chunk = []
    ...
    for msg in messages:
        ...
        else: current_chunk.append(msg); current_chunk_text_len += msg_len
    ...
    for i, chunk in enumerate(chunks):
        ...
        if chunk_summary: chunk_summaries.append(chunk_summary); let_log(f"Сводка чанка: {chunk_summary}")
        else: let_log(f"Не удалось создать сводку для чанка {i+1}")
    if not chunk_summaries: let_log(f"##### [{chat_id}] Не удалось создать сводки чанков. #####\n"); return
...
def compact_messages_llm(messages_to_compact: list[dict]) -> str: # Принимает группу сообщений (или одно сообщение) и просит LLM сжать их в единую суть.
...
def rerank_rag_results(chat_id: str, initial_results: dict) -> list[str]: # Заглушка
...
def prompt_assembler(chat_id: str, system_prompt: str, current_message: str, history: list[dict]) -> str:
    ...
    if global_summary: base_prompt_parts.append(f"{global_summary_marker}{global_summary}")
    if recent_summary: base_prompt_parts.append(f"{recent_summary_marker}{recent_summary}")
    ...
    if (history_was_truncated or last_global_id > 0) and new_global_tokens >= global_summary_tokens_percent * token_limit:
        ...
        messages_for_global = []
        if last_global_id > 0:
            messages_for_global.append({'id': last_global_id, 'role': 'global_summary', 'full_text': global_summary})  # Добавляем старую сводку как псевдо-сообщение
        ...
        for r in rows: messages_for_global.append({'id': r[0], 'role': r[1], 'full_text': r[2]})
    ...
    if (history_was_truncated or last_recent_id > 0) and new_recent_tokens >= recent_summary_tokens_percent * token_limit:
        ...
        for r in rows: messages_for_recent.append({'id': r[0], 'role': r[1], 'full_text': r[2]})
    ...
    if available_tokens_for_history <= 0: let_log(f"##### [{chat_id}] ВНИМАНИЕ: Нет места для истории. Пропускаем. #####")
            ...
            if (history_token_count + msg_tokens) <= available_tokens_for_history:
                ...
                if ENABLE_ON_THE_FLY_COMPRESSION and not msg.get('is_compressed'):
                    ...
                    if (history_token_count + msg_tokens_compressed) <= available_tokens_for_history:
                        ...
                        if msg.get('vector_id'): history_included_vector_ids.append(msg['vector_id'])
                    else: let_log(f"Даже сжатое сообщение ID {msg['id']} (токены: {msg_tokens_compressed:.0f}) не помещается. Усечение."); break
                else: let_log("Достигнут лимит токенов. Усечение."); break
    ...
    if history_was_truncated and available_tokens_for_rag_actual > 0:
        ...
        retrieved_texts = []
        if isinstance(initial_results, dict) and initial_results.get('ids') and initial_results['ids']: # ids могут быть списком списков (из-за query_embeddings=[...])
            ids_list = initial_results['ids'][0] if isinstance(initial_results['ids'][0], list) else initial_results['ids']
            if ids_list: # Построим запрос IN
        ...
        if retrieved_texts: # Создаём структуру, похожую на старую, с ключом documents
    ...
    else: let_log(f"##### [{chat_id}] RAG НЕ АКТИВИРОВАН. Условие: (Усечение: {history_was_truncated}, Доступно места: {available_tokens_for_rag_actual > 0}) #####")
    # Обеспечиваем нечётное количество сообщений в истории
    while len(history_strings_list) > 1 and len(history_strings_list) % 2 == 0: removed_msg = history_strings_list.pop(0); let_log(f"Удалено самое старое сообщение для сохранения нечетности: {removed_msg[:100]}...")
...
def rag_constructor(chat_id: str, system_prompt: str, current_message: str) -> str:
    ...
    final_prompt = prompt_assembler(chat_id=chat_id, system_prompt=system_prompt, current_message=current_message, history=history)
