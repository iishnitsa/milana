...
# --- Helper functions (copied from simple_web_search for independence) ---
...
def fetch_links_ddg(query, max_results=5):
    ...
    try:
        ...
        let_log(f'[fetch_links_ddg] Error: {e}')
    return results
...
def get_page_text(url):
    ...
    try:
        ...
        let_log(f'[get_page_text] Error: {e}')
        return ''
...
def generate_initial_queries(topic, prompt):
    ...
    try:
        ...
        let_log(f'[generate_initial_queries] Failed: {e}')
        raise
...
def generate_next_queries(topic, current_info_summary, gap_prompt, queries_prompt):
    ...
    try:
        ...
        let_log(f'[generate_next_queries] Gap analysis failed: {e}')
        return []
    ...
    try:
        ...
        let_log(f'[generate_next_queries] Query generation failed: {e}')
        return []
...
def synthesize_answer(topic, all_data, synthesis_prompt):
    ...
    try:
        ...
        let_log(f'[synthesize_answer] Failed: {e}')
        raise
...
def summarize_info(data):
    ...
    # Just concatenate first 2000 chars of each text to keep it manageable
    parts = []
    for url, text in data:
        parts.append(f"From {url}:\n{text[:1500]}")
    combined = "\n\n".join(parts)
    # Use text_cutter to compress further (but this is outside cacher)
...
def main(text):
    if not hasattr(main, 'attr_names'):
        ...
        main.generate_queries_prompt = (
            "You are an expert research assistant. Generate 3-5 specific search queries (one per line) "
            "that would help gather comprehensive information on the user's topic. "
            "Output only the queries, each on a new line. No numbering, no extra text."
        )
        main.gap_analysis_prompt = (
            "You are a research analyst. Analyze the information gathered so far and identify specific "
            "knowledge gaps that still need to be filled to provide a complete, well-researched answer "
            "to the original topic. List each gap on a new line as a clear, actionable question. "
            "If no significant gaps remain, output exactly the word 'NONE'."
        )
        main.followup_queries_prompt = (
            "You are a search query generator. Based on the identified knowledge gaps, generate 3-5 "
            "specific search queries (one per line) that would help fill those gaps. "
            "Output only the queries, each on a new line. No numbering, no extra text."
        )
        main.synthesis_prompt = (
            "You are a research assistant. Based on all the information provided by the user, "
            "produce a comprehensive, well-structured answer to the original question/topic. "
            "Include key facts, different perspectives if any, and cite sources by their URLs. "
            "Write in clear paragraphs, do not use markdown, be concise but thorough."
        )
    ...
    try:
        ...
        while iteration < MAX_ITERATIONS:
            ...
            if not new_data:
                let_log('[DeepResearch] No new data fetched, stopping.')
                break
            
            # If we have enough data (some threshold), we can optionally break early
            if total_pages_fetched >= MAX_TOTAL_PAGES:
                let_log(f'[DeepResearch] Reached max total pages ({MAX_TOTAL_PAGES}), stopping.')
                break
            ...
            if not next_queries:
                let_log('[DeepResearch] No more queries to generate, stopping iteration.')
                break
        ...
        if not all_data:
            let_log('[DeepResearch] No data collected at all')
        ...
        let_log(f'[DeepResearch] Unexpected error: {e}')
        return main.error_msg