...
def main(text: str) -> str:
    ...
    def is_command_banned(cmd: str) -> bool:
        cmd_lc = cmd.lower()
        if any(bad in cmd_lc for bad in BANNED): return True
        for pat in DANGEROUS_PATTERNS:
            if re.search(pat, cmd_lc): return True
        return False
    def is_path_safe(token: str, base: Path) -> bool:
        try: path = (base / token).resolve(); return str(path).startswith(str(base))
        except Exception: return False
    def validate_paths(command: str, work_dir: Path) -> bool:
        tokens = shlex.split(command, posix=False)
        for tok in tokens:
            if '\\' in tok or (':' in tok and not tok.startswith('%')):
                if not is_path_safe(tok, work_dir): return False
        return True
    def execute_command(command: str, work_dir: Path) -> str:
        try:
            result = subprocess.run(["cmd.exe", "/c", command], cwd=work_dir, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=60, encoding='utf-8')
            return result.stdout
        except subprocess.TimeoutExpired: return main.timeout_text
        except Exception as e: return f"{main.exception_text} {e}"
    command = text.strip()
    work_dir = get_agent_dir()
    os.makedirs(work_dir, exist_ok=True)
    if is_command_banned(command): return main.forbidden_text
    if not validate_paths(command, work_dir): return main.path_error_text
