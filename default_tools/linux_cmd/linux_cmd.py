...
def main(text: str) -> str:
    ...
    DANGEROUS_PATTERNS = [
        ...
        r'rm\s+-rf\s+/\*',           # rm -rf /*
        r'>\s*/dev/sd',              # > /dev/sda
        r'dd\s+if=.*\s+of=/dev/sd',  # dd if=... of=/dev/sdX
        r'\|\s*.*sh',                # | sh
        r':\(\)\s*{\s*:.*};\s*:',    # fork-bomb
    ...
    def execute_command(command: str, work_dir: Path) -> str:
        try:
            result = subprocess.run(["/bin/bash", "-c", command], cwd=work_dir, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=60, encoding='utf-8')
