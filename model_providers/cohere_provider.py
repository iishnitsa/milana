...
openai_base_url = ""
ollama_base_url = ""
openai_timeout = 0  # будет установлен из аргумента timeout функции connect
...
def find_ollama_context_size(model_data, model_name="unknown"):
    ...
    if parameters:
        ...
        if match: ctx = int(match.group(1)); let_log(f"Найден num_ctx в parameters: {ctx}"); return ctx
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if r.status_code == 429:
                try: error_data = r.json().get("error", {}); error_code = error_data.get("code"); error_msg = error_data.get("message", "").lower()
...
def connect(connection_string, timeout=30, _decrypted_token=None):
    ...
    # Устанавливаем глобальный таймаут из аргумента функции
    openai_timeout = timeout if timeout > 0 else 0
    ...
    # Если передан расшифрованный токен, используем его
    ...
    openai_session.headers.update({"Authorization": f"Bearer {api_key}", "Content-Type": "application/json", "User-Agent": "milana-openai-provider",})
    openai_session.emb_model = emb_model
    try: timeout_value = _get_timeout_value(); r = openai_session.get(f"{openai_base_url}/models", timeout=timeout_value); r.raise_for_status()
...
def create_embeddings(text):
    ...
    if ollama_session:
        try:
            if len(text) > 2000: let_log(f"create_embeddings (Ollama): обрезаем текст с {len(text)} до 2000 символов"); text = text[:2000]
