...
# Константы для прогрессивной задержки Ollama
...
def _make_request(api_url, payload):
    ...
    ТОЛЬКО ДЛЯ OPENAI, НЕ ДЛЯ OLLAMA.
    """
        ...
        try:
            ...
            if r.status_code == 429:
                ...
                if attempt < MAX_RETRIES:
                    ...
                    continue
                else: raise RuntimeError("balance end or extremely low RPM limit")
            ...
            if "balance end" in str(e) or "API Provider Error" in str(e): raise
...
def _ollama_request_with_backoff(api_url, json_payload):
    ...
    БЕЗ превентивной паузы и случайной задержки.
    """
...
def create_embeddings(text):
    ...
    if ollama_session:
        try:
            # Фильтрация длины для избежания ошибок контекста
            if len(text) > 2000:
                let_log(f"create_embeddings (Ollama): Текст слишком длинный ({len(text)} символов), обрезаем до 2000")
                text = text[:2000]
            payload = {
