...
def find_ollama_context_size(model_data, model_name="unknown"):
    ...
    for key in direct_keys:
        if key in model_info:
            try:
                ...
                if val > 0:
                    let_log(f"Найден контекст по ключу '{key}': {val}")
                    return val
    ...
    for key, value in model_info.items():
        if 'context_length' in key.lower() and isinstance(value, (int, float)):
            let_log(f"Найден контекст по ключу '{key}': {int(value)}")
            return int(value)
    ...
    if found_sizes:
        let_log(f"Жадный поиск нашёл контекст: {found_sizes[0]}")
        ...
        try:
            ...
            if r.status_code == 429:
                ...
                if error_code == "insufficient_quota" or "insufficient_quota" in error_msg:
                    let_log(f"CRITICAL: insufficient quota - {r.text}")
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                if wait_time is not None:
                    ...
                    if wait_time > OLLAMA_MAX_WAIT_TOTAL:
                        raise RuntimeError(f"Лимит квоты требует ожидания {wait_time}с, что превышает общий лимит {OLLAMA_MAX_WAIT_TOTAL}с")
            ...
            if attempt < OLLAMA_MAX_RETRIES:
                ...
                raise RuntimeError(f"Ollama ошибка соединения после {OLLAMA_MAX_RETRIES} попыток: {e}")
            ...
            if attempt < OLLAMA_MAX_RETRIES:
                ...
                raise RuntimeError(f"Ollama таймаут запроса после {OLLAMA_MAX_RETRIES} попыток: {e}")
            ...
            if attempt < OLLAMA_MAX_RETRIES:
                ...
                raise RuntimeError(f"Ollama сетевая ошибка после {OLLAMA_MAX_RETRIES} попыток: {e}")
        ...
        try:
            ...
            if ollama_emb_model not in available_models:
                ...
                if embed_models:
                    ...
                    let_log(f"Выбрана модель для эмбеддингов: {ollama_emb_model}")
                else:
                    # Если нет embed-моделей, используем первую доступную
                    if available_models:
                        ollama_emb_model = available_models[0]
                        let_log(f"Модель для эмбеддингов не найдена. Используем первую: {ollama_emb_model}")
                    else:
                        raise RuntimeError("Нет доступных моделей в Ollama")
            ...
            if show_response.status_code == 200:
                ...
                emb_token_limit = find_ollama_context_size(model_details, ollama_emb_model)
            else:
                emb_token_limit = 4095
