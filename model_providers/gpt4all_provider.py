...
def connect(connection_string, timeout=30):
    ...
    try:
        ...
        if requested_model and requested_model in available_models: default_chat_model = requested_model
            ...
            if requested_model: error_msg = f"Запрошенная модель GPT4All '{requested_model}' не найдена. Доступные: {available_models}"
            else: error_msg = f"Модель GPT4All не указана. Доступные: {available_models}"
        ...
        if ollama_emb_model not in ollama_available_models: error_msg = f"Запрошенная модель эмбеддингов Ollama '{ollama_emb_model}' не найдена. Доступные: {ollama_available_models}"; return [False, 0, tags, error_msg]
        ...
        try:
            ...
            if show_response.status_code == 200:model_details = show_response.json(); emb_token_limit = find_context_size(model_details, ollama_base_url, {})
            else: emb_token_limit = 4095
        except Exception as e: let_log(f"Ошибка при получении деталей модели эмбеддингов: {e}"); emb_token_limit = 4095
...
def _request_with_backoff(api_url, json_payload):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                if retry_after and retry_after.isdigit(): wait_time = int(retry_after)
                    ...
                    try:
                        ...
                        elif 'insufficient_quota' in err_msg or 'balance' in err_msg: raise RuntimeError("balance end")
                    except: pass
            ...
            if response.status_code == 402: raise RuntimeError("balance end")
            ...
            if "balance end" in str(e): raise
            if attempt < MAX_RETRIES: time.sleep(BASE_BACKOFF ** attempt); continue
            raise
...
def create_embeddings(text):
    ...
    while True:
        try:
            ...
            if attempt < MAX_RETRIES: time.sleep(BASE_BACKOFF ** attempt); attempt += 1; continue
            raise
        except Exception as e:
            if attempt < MAX_RETRIES: time.sleep(BASE_BACKOFF ** attempt); attempt += 1; continue
