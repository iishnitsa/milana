...
def sql_exec(query, params=(), fetchone=False, fetchall=False):
    # TODO: ошибка не кэшируется и возвращает None, может как служить маркером так и добавить проблем
    let_log('ОЧЕРЕДЬ')
    let_log(query)
...
def load_chat_settings(chat_id):
    ...
    # Кастомные модули
    custom_mods = sql_exec("SELECT adress FROM custom_mods WHERE enabled=1", fetchall=True)
    if custom_mods:
